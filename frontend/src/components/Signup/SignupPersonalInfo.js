import { Box, Divider, FormControl, FormHelperText, Grid, IconButton, Input, Stack, SvgIcon, Tooltip } from '@mui/joy';
import React from 'react';


const SignUpPersonalInfo = ({ inputData, setInputData, values, errors, touched, handleBlur, handleChange }) => {
  const google = () => {
  ...
  return (
    <Box>
      <Stack spacing={3}>
        ...
        <FormControl error={errors.firstName && touched.firstName ? true : false} fullWidth>
          <Input type='text' id='firstName' name='firstName' value={values.firstName} onChange={handleChange} onBlur={handleBlur} sx={{ backgroundColor: 'rgba(255, 255, 255, 0.2)', backdropFilter: 'blur(20px) saturate(180%)' }} error={errors.firstName && touched.firstName ? 'true' : false} required placeholder="First Name" />
          <FormHelperText sx={{ height: '1rem' }}>{errors.firstName && touched.firstName ? errors.firstName : (' ')}</FormHelperText>
        </FormControl>

        <FormControl error={errors.lastName && touched.lastName ? true : false} fullWidth >
          <Input type='text' id='lastName' name='lastName' value={values.lastName} onChange={handleChange} onBlur={handleBlur} sx={{ backgroundColor: 'rgba(255, 255, 255, 0.2)', backdropFilter: 'blur(20px) saturate(180%)' }} error={errors.lastName && touched.lastName ? 'true' : false} required placeholder="Last Name" />
          <FormHelperText sx={{ height: '1rem' }}>{errors.lastName && touched.lastName ? errors.lastName : (' ')}</FormHelperText>
        </FormControl>

        <FormControl error={errors.birthDate && touched.birthDate ? true : false} fullWidth sx={{ mb: 6 }}>
          <Input type='date' id='birthDate' name='birthDate' value={values.birthDate} onChange={handleChange} onBlur={handleBlur} sx={{ backgroundColor: 'rgba(255, 255, 255, 0.2)', backdropFilter: 'blur(20px) saturate(180%)' }} placeholder="Birth Date" />
          <FormHelperText sx={{ height: '1rem' }}>{errors.birthDate && touched.birthDate ? errors.birthDate : (' ')}</FormHelperText>
        </FormControl>

      </Stack>

      <Divider sx={{ mt: 3 }}>Social Media Sign In</Divider>
