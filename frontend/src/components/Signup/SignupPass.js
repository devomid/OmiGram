...
const SignupPass = ({ inputData, setInputData, values, errors, touched, handleBlur, handleChange }) => {
      ...
      <Stack spacing={3}>
        ...
        <Grid xs={18}>
          <FormControl error={errors.passwordConfirmation && touched.passwordConfirmation ? true : false}>
            <Input id='passwordConfirmation' name='passwordConfirmation' value={values.passwordConfirmation} onChange={handleChange} onBlur={handleBlur} sx={{ backgroundColor: 'rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(10px) saturate(180%)', border: '1px solid rgba(50, 50, 50, 0.3)' }} endDecorator={<IconButton onClick={() => setVisibility(!visibility)}>{visibility ? <VisibilityIcon /> : <VisibilityOffIcon />}</IconButton>} type={visibility ? 'password' : 'text'} placeholder='Confirm Password' />
            <FormHelperText sx={{ height: '1rem' }}>{errors.passwordConfirmation && touched.passwordConfirmation ? errors.passwordConfirmation : (' ')}</FormHelperText>
          </FormControl>
        </Grid>
      </Stack>

      <Divider sx={{ mt: 13.27 }}>Social Media Sign In</Divider>
